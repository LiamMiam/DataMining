...
MIN_USERS = 25 #nombre minimum d'utilisateurs différents pour garder un cluster
USER_COL = "user"

# Nombre d'utilisateurs uniques par cluster
users_per_cluster = df_valid_clusters.groupby("cluster")[USER_COL].nunique()

# Clusters qui passent le seuil
valid_cluster_ids = users_per_cluster[users_per_cluster >= MIN_USERS].index

# Filtrage final
df_valid_clusters = df_valid_clusters[df_valid_clusters["cluster"].isin(valid_cluster_ids)]
